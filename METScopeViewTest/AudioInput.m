...
static void AudioInputCallback(void *inUserData, AudioQueueRef inAQ, AudioQueueBufferRef inBuffer, 
	...
	if (err)
		NSLog(@"AudioInput: Failed to enqueue buffer: %d", (int)err);
}

- (void)setFreeze: (BOOL)setIsFreeze
{
...
- (BOOL)start {
	...
	if (err) {
		NSLog(@"AudioInput: Failed to start audio recording: %d", (int)err);
		result = NO;
	} else {
		isRunning = YES;
		result = YES;
...
- (BOOL)stop {
	...
	if (err) {
		NSLog(@"AudioInput: Failed to stop audio recording: %d", (int)err);
		result = NO;
	} else {
		err = AudioQueueFlush(queue);
		if (err) 
			NSLog(@"AudioInput: Failed to flush buffers: %d", (int)err);
		isRunning = NO;
		result = YES;
	}
