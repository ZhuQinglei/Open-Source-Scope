...
@synthesize channel1Enabled;
@synthesize channel2Enabled;
...
- (void)setDefaults {
    ...
    pthread_mutex_init(&dataMutex, NULL);   // Initialize data mutex
...
- (void)setDataAtIndex:(int)index withLength:(int)length xData:(float *)xx yData:(float *)yy color:(UIColor *)color lineWidth:(float)width {
    ...
    /* Lock the mutex so drawRect: doesn't access the plot data while it's being assigned */
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextRef contextChannel2 = UIGraphicsGetCurrentContext();
    CGPoint currentChannel2;
    
    pthread_mutex_lock(&dataMutex);
    
    int nWaveforms = (int)[plotData count];
    int nWaveformsChannel2 =(int)[plotData count];
    ...
    NSMutableArray *previousChannel2 = [[NSMutableArray alloc] initWithCapacity:nWaveformsChannel2];
...
//        for (int i = 1; i < plotResolution-2; i++) {
//            
//            for (int n = 0; n < nWaveformsChannel2; n++) {
//                
//                /* Current point: n^th waveform, i^th sample */
//                currentChannel2 = [self plotScaleToPixel:[plotData[n][i] CGPointValue]];
//                
//                CGContextBeginPath(contextChannel2);
//                
//                /* Set the color for this waveform */
//                CGContextSetStrokeColorWithColor(contextChannel2, ((UIColor *)[plotColors objectAtIndex:n]).CGColor);
//                CGContextSetLineWidth(contextChannel2, [[lineWidths objectAtIndex:n] floatValue]);
//                
//                /* Append line from previous point to current point */
//                CGContextMoveToPoint(contextChannel2, [previousChannel2[n] CGPointValue].x, [previousChannel2[n] CGPointValue].y);
//                CGContextAddLineToPoint(contextChannel2, currentChannel2.x, currentChannel2.y);
//                
//                /* Draw */
//                CGContextStrokePath(contextChannel2);
//                
//                /* Current point becomes the previous point */
//                [previousChannel2 replaceObjectAtIndex:n withObject:[NSValue valueWithCGPoint:currentChannel2]];
//            }
//        }
    }
